...
# Funções auxiliares leves mantidas no topo
...
class CotacaoModel:
    ...
    def processar_base_dados(self, caminho_entrada):
        # Pandas é carregado APENAS quando o usuário clica para importar a planilha
        import pandas as pd
    ...
    def gerar_planilha(self, arquivo_saida, ordem_desejada):
        # OpenPyXL é carregado APENAS no momento de exportar a planilha
        import openpyxl
            ...
            for col in range(1, num_transp + 2):
                ...
                if col >= 2:
                    ...
                    if transp_nome == "TRANSBUIATTE":
                        cfg = ConfigManager()
                        v_acrescimo_cfg = cfg.get("frete_transbuiatte_acrescimo")
                        v_base_manaus_cfg = cfg.get("frete_transbuiatte_base_manaus")

                        valor_acrescimo = float(v_acrescimo_cfg) if v_acrescimo_cfg is not None else 1100.0
                        valor_base_manaus = float(v_base_manaus_cfg) if v_base_manaus_cfg is not None else 4500.0

                        num_entregas = len(grupo[['CIDADE_calc', 'CLIENTE_calc']].drop_duplicates())
                        acrescimo = max(0, num_entregas - 1) * valor_acrescimo
                        cidades_bloco = grupo['CIDADE_calc'].astype(str).str.upper().str.strip().tolist()
                        is_manaus = any('MANAUS' in c for c in cidades_bloco)

                        if is_manaus:
                            formula_transb = f"={valor_base_manaus}+{acrescimo}" if acrescimo > 0 else f"={valor_base_manaus}"
                        else:
